...
import { Flex, Container, Link, Text, UnorderedList, ListItem } from '@chakra-ui/core';
...
const Index = ({data}) => {
    ...
    return (
        <Container h="100vh" centerContent>
            ...
            <Text fontSize="6xl" mb={6}>Statamic Blog</Text>
            <UnorderedList>
            {posts.map((post) => (
                <ListItem>
                    <Link as={GatsbyLink} to={post.slug}> To {post.slug}</Link>
                </ListItem>
            ))}
            </UnorderedList>
            </Flex>
